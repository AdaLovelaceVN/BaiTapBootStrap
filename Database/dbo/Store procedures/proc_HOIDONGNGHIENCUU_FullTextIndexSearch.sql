...
BEGIN
	...
	IF @HoTen IS NOT NULL
		BEGIN 
			...
			AND (NgayQuyetDinh >= @QuyetDinhTuNgay  OR @QuyetDinhTuNgay IS NULL) 
			AND (NgayQuyetDinh <= @QuyetDinhDenNgay  OR @QuyetDinhDenNgay IS NULL)
	...
	ELSE
		BEGIN
			...
			WHERE 
			(NgayQuyetDinh >= @QuyetDinhTuNgay  OR @QuyetDinhTuNgay IS NULL) 
			AND (NgayQuyetDinh <= @QuyetDinhDenNgay  OR @QuyetDinhDenNgay IS NULL)
			AND (UPPER(LTRIM(RTRIM(SoQuyetDinh))) LIKE '%' +UPPER(LTRIM(RTRIM(@SoQuyetDinh)))+ '%' OR @SoQuyetDinh IS NULL)
			AND (@UserId = DT.UserId OR @UserId IS NULL)
		END	
